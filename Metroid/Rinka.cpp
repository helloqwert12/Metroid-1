...
Rinka::Rinka(int x, int y, GVector2 velocity) : BaseObject(RINKA)
{
	...
	_sprite->setFrameRect(SpriteManager::getInstance()->getSourceRect(eID::ENEMY, "rinka_01"));
	_sprite->setPosition(x, y);

	_initPosition = GVector2(x, y);

	_animation = new Animation(_sprite, 0.07f);
	_animation->addFrameRect(eID::ENEMY, "rinka_01", "rinka_02", "rinka_03", NULL);
...
void Rinka::update(float deltatime)
{
	if (_hitPoint > 0)
	{
...
void Rinka::draw(LPD3DXSPRITE spriteHandle, Viewport* viewport)
{
	if (_hitPoint > 0)
...
void Rinka::wasHit(int hitPoint)
{
...
GVector2 Rinka::getVelocity()
{
	auto movement = (Movement*)this->_componentList["Movement"];
	return movement->getVelocity();
}

float Rinka::checkCollision(BaseObject* object, float dt)
{
	return 0;
}
